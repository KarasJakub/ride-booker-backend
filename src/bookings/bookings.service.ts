...
export class BookingsService {
  ...
  // Admin view - bookings by location
  async findByLocation(locationId: string, userId: string, userRole: string) {
    ...
    if (userRole === 'ORG_ADMIN') {
      ...
      if (user?.organizationId !== location.organizationId) {
        throw new ForbiddenException('You dont have access to this location bookings');
      }
    }

    if (userRole === 'BRANCH_ADMIN') {
      if (location.branchAdminId !== userId) {
        throw new ForbiddenException('You can only view bookings for your location');
      }
    }
  ...
  async create(dto: CreateBookingDto, userId: string) {
    // 1. Check slot availability and time
    ...
    // 2. Check if user already has a booking in this time slot
    ...
    if (conflict) {
      throw new BadRequestException('You already have a booking in this time slot');
    }

    // 3. Create booking and lock slot
  ...
  async updateStatus(
    ...
    if (userRole === 'ORG_ADMIN') {
      const user = await this.prisma.user.findUnique({ where: { id: userId } });
      if (user?.organizationId !== location.organizationId) {
        throw new ForbiddenException('You dont have access to this booking');
      }
    }
  ...
  // User can cancel only own bookings that are pending or confirmed
